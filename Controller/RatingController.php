...
class RatingController extends Controller
{
    public function showRateAction($id, Request $request)
    {
        ...
        return $this->render('DCSRatingBundle:Rating:star.html.twig', array(
            'rating' => $rating,
            'rate'   => $rating->getRate(),
            'maxValue' => $this->getParameter('dcs_rating.max_value'),
        ));
    }
    ...
    public function controlAction($id, Request $request)
    {
        ...
        if (false === $this->get('security.authorization_checker')->isGranted($rating->getSecurityRole())) {
            $viewName = 'star';
        } else {
            // check if the voting system allows multiple votes. Otherwise
            // check if the user has already expressed a preference
            if (!$this->getParameter('dcs_rating.unique_vote')) {
                $viewName = 'choice';
            } else {
                $vote = $this->get('dcs_rating.manager.vote')
    ...
    public function addVoteAction($id, $value, Request $request)
    {
        ...
        if (null === $rating->getSecurityRole() || false === $this->get('security.authorization_checker')->isGranted($rating->getSecurityRole())) {
        ...
        if (null === $redirectUri = $request->headers->get('referer', $rating->getPermalink())) {
            $pathToRedirect = $this->getParameter('dcs_rating.base_path_to_redirect');
